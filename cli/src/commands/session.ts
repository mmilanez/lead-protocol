...
export function registerSessionCommand(program: Command): void {
  ...
  session.command("open").requiredOption("--topic <text>", "one-line session topic")
    ...
    .action((opts: PairOptions & { topic: string }) => { try { const receipt = openSession(opts); if (opts.json) console.log(JSON.stringify(receipt, null, 2)); else { ui.success(`Opened ${receipt.sessionId}; receipt written for ${receipt.pair.actor}/${receipt.pair.agent}`); console.log(); console.log(ui.heading("Previous operational state")); console.log(ui.label("Status", receipt.previousHandoff?.status ?? "No previous handoff")); console.log(ui.label("Pending Step", receipt.previousHandoff?.pending_step ?? "None")); console.log(ui.label("Blockers", receipt.previousHandoff?.blockers_context ?? "None")); console.log(ui.label("Open Threads", receipt.previousHandoff?.open_threads ?? "None")); } } catch (e) { fail(e); } });
