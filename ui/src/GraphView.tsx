...
import { Background, Controls, MarkerType, MiniMap, Position, ReactFlow, useNodesState, type Edge, type Node } from '@xyflow/react'
...
function buildGraph(data: GraphProtocolData, layout: Layout) {
  ...
  const addNode = (kind: Kind, key: string, title: string, subtitle: string, details: [string, string][]) => {
    ...
    if (!nodeMap.has(nodeId)) nodeMap.set(nodeId, { id: nodeId, position: { x: 0, y: 0 }, width: layout === 'organic' ? 215 : 190, height: layout === 'organic' ? 58 : 55, sourcePosition: Position.Right, targetPosition: Position.Left, className: `protocol-node node-${kind}`, data: { label: label(kind, title, subtitle), title, subtitle, kind, details } })
    return nodeId
  }
